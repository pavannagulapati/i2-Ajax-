...
$(document).ready(function () {
  $('[cell-pc^="[-cl"]').click(function () {
    //For Transfering Page Contents
    var pc = $(this).attr('cell-pc').match(/[-][c][l](\d{1,})[^\w\*](\d{1,})/i);
    ...
    $.get('' + pcPage[i] + '', function (data) {
      ...
      $('[cell-pc^="[-pc' + pc[1] + '(' + pc[2] + '):-re"]').empty().append(localStorage.getItem('ColorStorage' + pc[1] + '(' + pc[2] + ')'));
      $('[cell-pc^="[-pc' + pc[1] + '(' + pc[2] + '):-re"]').slideDown()
    });
  })
})
$(document).ready(function () {
  jQuery(function () {
    jQuery('[cell-pc*="-a"]').trigger('click');
  });
})
